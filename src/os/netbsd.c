/*
 * NetBSD OS layer - non-inline implementations for every facility
 * declared in include/jemalloc/internal/os/netbsd.h.
 */

#define JEMALLOC_OS_NETBSD_C_
...
os_vm_reserve(void *hint, size_t size, size_t align, bool *commit) {
	...
	/*
	 * On NetBSD PAGE for a platform is defined to the maximum page size
	 * of all machine architectures for that platform, so that we can use
	 * the same binaries across all machine architectures. Request
	 * MAP_ALIGNED when align or PAGE exceeds the actual os_page.
	 */
	if (align > os_page || PAGE > os_page) {
		unsigned int a = ilog2(MAX(align, PAGE));
		flags |= MAP_ALIGNED(a);
	}
...
os_vm_boot(void) {
	...
	/* NetBSD kernel overcommits unconditionally. */
	os_overcommits = true;
