...
size_t pac_batched_alloc_retained_size(size_t size) {
	if (size < USIZE_GROW_SLOW_THRESHOLD) {
		// this should actually be size * opt_lg_extent_max_active_fit, change later
	...
	if (caching_size >= SC_LARGE_MAXCLASS) {
		caching_size = size;
	}
	return caching_size;
}
...
pac_alloc_real(tsdn_t *tsdn, pac_t *pac, ehooks_t *ehooks, size_t size,
	...
	if (edata == NULL) {
		...
		if (config_limit_usize_gap && (maps_coalesce || opt_retain)) {
			size_t caching_size = pac_batched_alloc_retained_size(size);
			edata = ecache_alloc_grow(tsdn, pac, ehooks,
			    &pac->ecache_retained, NULL, caching_size, alignment,
			    zero, guarded);

			if (config_stats && edata != NULL) {
				atomic_fetch_add_zu(&pac->stats->pac_mapped, caching_size,
				    ATOMIC_RELAXED);
			}

			if (edata != NULL && caching_size > size) {
				edata_t *trail = extent_split_wrapper(tsdn, pac, ehooks, edata,
				    size, caching_size - size, /* holding_core_locks */ false);
				if (trail == NULL) {
