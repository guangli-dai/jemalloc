 ...
 * VM
 *
 * Linux's VM facility: mmap/munmap with overcommit-aware reservation (the
 * alignment-trim slow path is orchestrated in src/pages.c), madvise variants
 * for purge / hugify / dump-ability / batch purge, mprotect guard pages,
 * prctl(PR_SET_VMA_ANON_NAME) under JEMALLOC_PAGEID, and /proc + /sys reads
 * for boot-time state. Out-of-line bodies live in src/os/linux.c; the flags
 * let portable src/pages.c drop whole branches at compile time. State:
...
#  define OS_VM_HAS_HUGE_COLLAPSE 1
#else
#  define OS_VM_HAS_HUGE_COLLAPSE 0
#endif

#ifdef JEMALLOC_HAVE_PROCESS_MADVISE
#  define OS_VM_HAS_BATCH_PURGE 1
#else
...
#define OS_VM_HAS_THP_INFO 1

#if defined(JEMALLOC_HAVE_PRCTL) && defined(JEMALLOC_PAGEID)
#  define OS_VM_HAS_SET_NAME 1
#else
 ...
 *
 * Linux uses the POSIX pthread backend: this section selects it, and the
 * typedefs + inline bodies (os_mutex_lock/unlock/trylock/destroy, os_cond_*,
 * os_sigmask_*) come from posix_common.h. Only os_mutex_init is Linux-local.
 * malloc_mutex_t embeds os_mutex_t as its `lock` field; the typedef +
 * OS_MUTEX_INITIALIZER preserve the pre-refactor layout so
 * MALLOC_MUTEX_INITIALIZER stays byte-identical.
 *
 * Capability flags:
 ...
 * Process
 *
 * Process id and fork-handler registration. Linux registers fork hooks via
 * pthread_atfork (out-of-line in src/os/linux.c; one-shot at boot).
 *
 * Capability flags:
