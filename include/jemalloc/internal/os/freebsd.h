 ...
 * VM
 *
 * FreeBSD's VM facility: mmap with MAP_EXCL/MAP_ALIGNED for fast aligned
 * reservations (the alignment-trim slow path is orchestrated in
 * src/pages.c), sysctl(vm.overcommit) for the overcommit decision, madvise
 * variants for purge, and mprotect guard pages. madvise(2) is known to
 * work, so the lazy-purge runtime probe is skipped via
 * OS_VM_LAZY_PURGE_NEEDS_RUNTIME_CHECK = 0. Out-of-line bodies live in
 * src/os/freebsd.c; the flags let portable src/pages.c drop whole branches
 * at compile time. State: `os_overcommits` (boot cache).
 *
 * Capability flags:
...
#if defined(JEMALLOC_MADVISE_DONTDUMP) || defined(JEMALLOC_MADVISE_NOCORE)
#  define OS_VM_HAS_DONTDUMP 1
#else
...
/* FreeBSD's madvise(2) is known to work; skip the lazy-purge probe. */
#define OS_VM_LAZY_PURGE_NEEDS_RUNTIME_CHECK 0
 ...
 *
 * FreeBSD uses the POSIX pthread backend: this section selects it, and the
 * typedefs + inline bodies (os_mutex_lock/unlock/trylock/destroy, os_cond_*,
 * os_sigmask_*) come from posix_common.h. Only os_mutex_init is FreeBSD-local.
 * When libthr is in use (JEMALLOC_MUTEX_INIT_CB), the per-mutex init dance is
 * special: it relies on the libthr-private _pthread_mutex_init_calloc_cb hook
 * to avoid recursion through calloc during bootstrap. That postpone-state
 * machinery stays in src/mutex.c; os_mutex_init here provides only the
 * non-bootstrap pthread init path.
 *
 * Capability flags:
 ...
 * Thread
 *
 * OS-level thread identity. FreeBSD does not expose a gettid-style id here.
 *
 * Capability flags:
 ...
 * DSS / sbrk
 *
 * Data-segment growth via sbrk(2), used by src/extent_dss.c. FreeBSD
 * provides sbrk(2); the capability flag mirrors JEMALLOC_DSS. Out-of-line
 * body in src/os/freebsd.c.
 *
 * Capability flags:
