 ...
 * VM
 *
 * Solaris's VM facility: mmap/munmap (the kernel always overcommits, so there
 * is no overcommit knob and the alignment-trim slow path is orchestrated in
 * src/pages.c), memcntl(MC_HAT_ADVISE) for hugepage hints, and
 * JEMALLOC_MAPS_COALESCE for forced purge (mmap MAP_FIXED overlay). Out-of-line
 * bodies live in src/os/solaris.c; the flags let portable src/pages.c drop
 * whole branches at compile time. State: `os_overcommits` (boot cache).
 *
 * Capability flags:
 ...
 *
 * Solaris uses the POSIX pthread backend: this section selects it, and the
 * typedefs + inline bodies (os_mutex_lock/unlock/trylock/destroy, os_cond_*,
 * os_sigmask_*) come from posix_common.h. Only os_mutex_init is Solaris-local.
 *
 * Capability flags:
 ...
 * Thread
 *
 * OS-level thread identity. Solaris exposes no kernel thread id usable here.
 *
 * Capability flags:
...
JEMALLOC_ALWAYS_INLINE int
os_cpu_current(void) {
#if defined(JEMALLOC_HAVE_RDTSCP)
	unsigned int ecx;
 ...
 * Process
 *
 * Process id and fork-handler registration. Solaris registers fork hooks via
 * pthread_atfork (out-of-line in src/os/solaris.c; one-shot at boot).
 *
 * Capability flags:
