...
JEMALLOC_ALWAYS_INLINE rtree_contents_t
rtree_leaf_elm_read_fast(tsdn_t *tsdn, rtree_t *rtree, rtree_leaf_elm_t *elm,
    bool dependent) {
#ifdef RTREE_LEAF_COMPACT
	uintptr_t bits = rtree_leaf_elm_bits_read(tsdn, rtree, elm, dependent);
	/* On the fast path, do not read usize. */
	rtree_contents_t contents = rtree_leaf_elm_bits_decode(bits, false);
	return contents;
#else
	rtree_contents_t contents;
...
rtree_metadata_try_read_fast(tsdn_t *tsdn, rtree_t *rtree, rtree_ctx_t *rtree_ctx,
	...
	assert(elm != NULL);
	*r_rtree_metadata = rtree_leaf_elm_read_fast(tsdn, rtree, elm,
	    /* dependent */ true).metadata;
	return false;
}
